    ...
    <div className="min-h-screen bg-background pt-28 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div
          ...
          className="mb-12"
        >
          <h1 className="section-title text-white">{t('favorites.title')}</h1>
          <p className="text-muted mt-2">{t('favorites.subtitle', { count: favProperties.length })}</p>
        </motion.div>
              ...
              return (
                ...
                >
                  <Link to={`/properties/${p.id}`} className="card-hover group block rounded-2xl overflow-hidden">
                    ...
                    <div className="p-6">
                      <p className="text-muted text-xs mb-1"><i className="fa-solid fa-location-dot text-gold-500 mr-1" />{location}</p>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xs text-muted">{p.bedrooms} {t('favorites.beds')} | {p.bathrooms} {t('favorites.baths')}</span>
                      </div>
                    </div>
                  </Link>
                </motion.div>
