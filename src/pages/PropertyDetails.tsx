    ...
    return (
      <div className="pt-32 text-center min-h-screen bg-background">
        <div className="card p-12 max-w-md mx-auto rounded-2xl">
          ...
          <h2 className="text-xl text-white mb-4">{t('propertyDetails.notFound')}</h2>
    ...
    <div className="min-h-screen bg-background pt-24 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="mb-8">
          <Link to="/properties" className="inline-flex items-center gap-2 text-muted hover:text-gold-500 transition-colors text-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3 space-y-6">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="relative rounded-3xl overflow-hidden card-hover cursor-pointer group h-[400px] md:h-[500px]"
              ...
              {isSold && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                  <div className="text-center">
                    <i className="fa-solid fa-ban text-6xl text-red-400/80 mb-4" />
                    <p className="text-4xl font-bold text-red-400 uppercase tracking-widest">{t('propertyDetails.soldBadge')}</p>
                  </div>
                </div>
              )}
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity p-4 bg-white/10 backdrop-blur-md rounded-full">
                  <i className="fa-solid fa-expand text-white text-xl" />
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={i} className="rounded-2xl overflow-hidden h-24 md:h-32 cursor-pointer card-hover" onClick={() => setLightbox(img)}>
            ...
            {features.length > 0 && (
              <div className="card p-8 rounded-2xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {features.map((feat) => (
                    <div key={feat} className="flex items-center gap-3 bg-secondary rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {property.floor && (
                    <div className="bg-secondary rounded-xl p-4">
                      <p className="text-xs text-muted uppercase tracking-wider mb-1">{t('propertyDetails.floor')}</p>
                      <p className="text-white font-medium">{property.floor}</p>
                    </div>
                  )}
                  {property.occupancy && (
                    <div className="bg-secondary rounded-xl p-4">
                      <p className="text-xs text-muted uppercase tracking-wider mb-1">{t('propertyDetails.occupancy')}</p>
                      <p className="text-white font-medium">{property.occupancy} {t('propertyDetails.persons')}</p>
                    </div>
                  )}
                  {property.soundInsulation && (
                    <div className="bg-secondary rounded-xl p-4">
                      <p className="text-xs text-muted uppercase tracking-wider mb-1">{t('propertyDetails.soundInsulation')}</p>
                      <p className="text-white font-medium">{property.soundInsulation}</p>
                    </div>
                  )}
                  {property.daylightFactor && (
                    <div className="bg-secondary rounded-xl p-4">
                      <p className="text-xs text-muted uppercase tracking-wider mb-1">{t('propertyDetails.daylightFactor')}</p>
                      <p className="text-white font-medium">{property.daylightFactor}</p>
                    </div>
                  )}
                  {property.ceilingHeight && (
                    <div className="bg-secondary rounded-xl p-4">
                      <p className="text-xs text-muted uppercase tracking-wider mb-1">{t('propertyDetails.ceilingHeight')}</p>
                      <p className="text-white font-medium">{property.ceilingHeight}</p>
                    </div>
                  )}
                </div>
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-28 space-y-6">
              ...
              >
                <p className="text-4xl font-bold text-gold-500 mb-6">${property.price.toLocaleString(locale)}</p>

                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="text-center bg-secondary rounded-xl p-4">
                    <i className="fa-solid fa-bed text-gold-500 text-xl" />
                    <p className="text-white font-bold mt-1">{property.bedrooms}</p>
                    <p className="text-muted text-xs">{t('propertyDetails.bedrooms')}</p>
                  </div>
                  <div className="text-center bg-secondary rounded-xl p-4">
                    <i className="fa-solid fa-bath text-gold-500 text-xl" />
                    <p className="text-white font-bold mt-1">{property.bathrooms}</p>
                    <p className="text-muted text-xs">{t('propertyDetails.bathrooms')}</p>
                  </div>
                  <div className="text-center bg-secondary rounded-xl p-4">
                    <i className="fa-solid fa-ruler-combined text-gold-500 text-xl" />
                    <p className="text-white font-bold mt-1">{property.area}</p>
                    <p className="text-muted text-xs">{t('propertyDetails.area')}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <button
                    ...
                    disabled={isSold}
                    className={`w-full flex items-center justify-center gap-2 text-base py-4 rounded-xl font-medium transition-all duration-300 ${
                      isSold
                  ...
                  <a
                    ...
                    className="flex items-center justify-center gap-3 w-full py-4 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 hover:bg-green-500/20 transition-all duration-300 font-medium text-base"
                  >
                  ...
                  <a
                    ...
                    className="flex items-center justify-center gap-3 w-full py-4 rounded-xl bg-gold-500/10 border border-gold-500/30 text-gold-500 hover:bg-gold-500/20 transition-all duration-300 font-medium text-base"
                  >
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-white text-sm font-medium truncate group-hover:text-gold-500 transition-colors">{rpTitle}</p>
                            <p className="text-gold-500 font-bold text-sm mt-1">${rp.price.toLocaleString()}</p>
                          </div>
                        </Link>
                      );
                    })}
      ...
      {lightbox && (
        <div className="fixed inset-0 bg-background/95 z-50 flex items-center justify-center p-8" onClick={() => setLightbox(null)}>
          <img src={lightbox} className="max-w-full max-h-full object-contain rounded-2xl" />
          <button className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white hover:bg-gold-500 transition-all" onClick={() => setLightbox(null)}>
      ...
      {showBooking && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowBooking(false)}>
          <div className="card p-8 max-w-md w-full rounded-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-heading text-2xl font-bold text-white mb-4">
              <i className="fa-regular fa-calendar text-gold-500 mr-3" />
              {t('propertyDetails.bookNow')}
            </h3>
            <p className="text-muted mb-6">{t('propertyDetails.bookingMessage', { title })}</p>
            {bookingMsg && <p className="text-red-400 text-sm text-center mb-4">{bookingMsg}</p>}
            <div className="flex gap-4 justify-end">
              <button onClick={() => setShowBooking(false)} className="btn-outline">{t('admin.cancel')}</button>
              <button onClick={() => doBooking()} className="btn-primary">{t('propertyDetails.confirm')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showContact && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => { setShowContact(false); setContactStatus(''); }}>
          <div className="card p-8 max-w-md w-full rounded-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            {contactStatus && (
              <p className={`text-sm text-center mb-4 ${contactStatus === 'ok' ? 'text-green-400' : 'text-red-400'}`}>
            ...
            <form className="space-y-4" onSubmit={async (e) => {
              ...
              <div className="flex gap-4 justify-end pt-2">
                <button type="button" onClick={() => { setShowContact(false); setContactStatus(''); }} className="btn-outline">{t('admin.cancel')}</button>
